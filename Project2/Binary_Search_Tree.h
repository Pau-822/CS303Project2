...
public:
	...
	virtual string decode(string code);
...
private:
	...
	virtual bool insert(BTNode<Item_Type>*& local_root,
		const Item_Type& item, string code);
...
template<typename Item_Type>
bool Binary_Search_Tree<Item_Type>::insert(const Item_Type& item,string code) {
	return insert(this->root, item,code);
}


template<typename Item_Type>
bool Binary_Search_Tree<Item_Type>::insert(BTNode<Item_Type>*& local_root, const Item_Type& item,string code) {
	
	
	BTNode<Item_Type>* current_node = local_root;
	for (int i = 0; i < code.length(); i++)
	{
		if (code[i] == '.') {
			if (current_node->left == NULL) {
				current_node->left = new BTNode<Item_Type>("");
			}
			current_node = current_node->left;
		}
		else if (code[i] == '_') {
			if (current_node->right == NULL) {
				current_node->right = new BTNode<Item_Type>("");
			}
...
template<typename Item_Type>
string Binary_Search_Tree<Item_Type>::decode(string code) {


	BTNode<Item_Type>* current_node = this->root;
	for (int i = 0; i < code.length(); i++)
	{
		if (code[i] == '.') {			
			current_node = current_node->left;
